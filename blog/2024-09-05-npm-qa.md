...
-   `npm i`: (or `npm install`) installs all dependencies and can add individual dependencies (like `npm i @augment-vir/common`).
-   `npm ci`: removes `node_modules` and installs all dependencies _exactly_ as defined within `package-lock.json`.
    -   this should _only_ be used (never `npm i`) in automated or deploy environments.
    -   it is also good to use in local dev to make sure your dependencies are up to date.
